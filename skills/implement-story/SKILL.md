...
description: Implement an explicitly authorized, approved JStack engineering plan in a local repository with incremental checks and resumable checkpoints. Use after plan approval or for an authorized implementation-fix pass; do not use for planning, independent review, final completion, Git history, or remote actions.
---
...
3. Run the check through the host; JStack does not execute it.
4. After directly observing success, use `jstack state record-validation` with a concise cumulative summary, the captured fingerprint, and `--confirm-validation-succeeded`.
...
When the planned local behavior and appropriate focused tests are implemented, inspect the complete local diff for scope and accidental edits. Have the coordinating agent save a complete checkpoint with status `in-review`, no active implementation unit, and `jstack-review` as the exact next phase. Do not use `state complete` and do not perform the independent review yourself.
