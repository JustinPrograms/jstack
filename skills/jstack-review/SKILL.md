...
4. If `.jstack/checkpoint.md` exists, verify that its resolved path stays inside this worktree and the target is a regular file before reading. If unsafe, report the unavailable checkpoint context and review the actual change without reading its target. Otherwise read it as potentially stale context for task intent, decisions, claimed progress, validation, blockers, and approvals. Do not treat it as evidence that code exists, a check passed, an approval remains operative, or work is complete; reconcile every relevant claim against the repository and current conversation.
...
Use the actual change set and task symbols to guide focused filename/content search (`rg --files`, `rg`, Git search, or available host tools), then follow relevant callers and tests. Check applicable hidden instructions/configuration and relevant untracked files explicitly. Treat ignored/generated-directory exclusions as defaults, not reasons to omit in-scope behavior. Bound excerpts while retaining criteria and instructions; report truncated searches, inaccessible files, and unresolved coverage rather than treating a partial inspection as a clean review.

Before reporting, recheck the selected base, criteria, status, and relevant file contents against what you reviewed. HEAD and a modified-file list alone do not identify the reviewed change. If the target changed during the pass, revisit affected findings and coverage; if it keeps moving, report the observed scope and unfinished review. Prior validation or review conclusions are historical when their execution, inputs, or criteria cannot be verified. Do not renew them from a checkpoint claim or unchanged timestamp.
