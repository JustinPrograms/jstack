---
name: resume-story
description: Recover the latest successfully persisted JStack story state, validate it against current Git state, and continue from its saved next action. Use after compaction, usage limits, or a new agent session.
---
...
2. Prefer explicit `--workspace <workspace-id> --story <story-id>`. If identity is absent, run `jstack state list --repo <absolute-repository-path> --json` and continue only when exactly one valid bundle matches. If none or several match, show safe candidate identifiers and ask.
3. Read the selected bundle with `jstack state show` before broad repository exploration.
...
- use `jstack-review` for independent inspection of an `in-review` change set, or `implement-story` for a separately authorized fix pass; and
